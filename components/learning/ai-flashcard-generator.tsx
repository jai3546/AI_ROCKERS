...
export function AIFlashcardGenerator({
  ...
  const hasSourceContent = Boolean(defaultSourceContent.trim())
  const resolvedTopic = defaultTopic || defaultCustomSubject
  const materialSubject = defaultSubject || "General"
  const [subject, setSubject] = useState(
    hasSourceContent ? materialSubject : (defaultCustomSubject ? "custom" : (defaultSubject || ""))
  )
  const [customSubject, setCustomSubject] = useState(
    hasSourceContent ? resolvedTopic : (defaultCustomSubject || "")
  )
  ...
  const subjectOptions = {
    "General": ["General", "Math", "Science", "English", "Social Studies", "Computer Science"],
  ...
  const generateFlashcards = async () => {
    const selectedSubject = hasSourceContent
      ? materialSubject
      : (subject === "custom" ? customSubject : subject)
    const selectedTopic = hasSourceContent
      ? resolvedTopic
      : (subject === "custom" ? customSubject : selectedSubject)
    ...
    if (hasSourceContent && !defaultSourceContent.trim()) {
      setError("No lesson content available to generate flashcards from.")
      return
    }
  ...
  useEffect(() => {
    if (autoGenerate && defaultSourceContent.trim() && !autoGeneratedRef.current) {
      autoGeneratedRef.current = true
      void generateFlashcards()
    }
  }, [autoGenerate, defaultSourceContent])
        ...
        <div className="space-y-4">
          {hasSourceContent ? (
            <div className="space-y-3 rounded-lg border border-primary/20 bg-primary/5 p-3 text-sm">
              <p className="font-medium text-primary">Generating from your uploaded material</p>
              <p><span className="text-muted-foreground">Subject:</span> {materialSubject}</p>
              {resolvedTopic ? (
                <p><span className="text-muted-foreground">Topic:</span> {resolvedTopic}</p>
              ) : null}
            </div>
          ) : (
            <>
