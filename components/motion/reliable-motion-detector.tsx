...
export function ReliableMotionDetector({
  onMotionDetected,
  autoTracking = false,
  className = "",
  language = "en",
  ...
  const detectionHistoryRef = useRef<{time: number, detected: boolean}[]>([])
    ...
    const detectMotion = () => {
      ...
      try {
        ...
        if (video.readyState === video.HAVE_ENOUGH_DATA && 
          ...
          // Calculate stable detection state
          const history = detectionHistoryRef.current
          const detectionCount = history.filter(d => d.detected).length
          const nonDetectionCount = history.length - detectionCount
          
          // Need multiple consecutive detections to change state
          let stableInFrame = inFrame // Start with current state
          
          if (detectionCount > history.length * 0.7) {
            // 70% or more detections = person is in frame
            stableInFrame = true
          } else if (nonDetectionCount > history.length * 0.8) {
            // 80% or more non-detections = person is not in frame
            stableInFrame = false
          }
          
          // Draw detection status on canvas
          context.fillStyle = stableInFrame ? 'rgba(0, 255, 0, 0.3)' : 'rgba(255, 0, 0, 0.3)'
          context.fillRect(0, 0, 100, 30)
          context.fillStyle = '#ffffff'
          context.font = '14px Arial'
          context.fillText(stableInFrame ? 'In Frame' : 'Out of Frame', 10, 20)
          
          // Update state only if it changed
          if (inFrame !== stableInFrame) {
            setInFrame(stableInFrame)
            setConfidence(stableInFrame ? 90 : 10)
            
            // Call callback
            if (onMotionDetected) {
              onMotionDetected({
                inFrame: stableInFrame,
                confidence: stableInFrame ? 90 : 10,
                timestamp: new Date(),
                message: stableInFrame ? translations.stayInFrame[language] : translations.returnToFrame[language]
              })
            }
          }
        }
